...
class PoliceNormModel:
    ...
    def create_crime_rate(self):
        ...
        police['강도검거율'] = (police['강도 검거'] / police['살인 발생']) * 100
        police['강간검거율'] = (police['강간 검거'] / police['살인 발생']) * 100
        police['절도검거율'] = (police['절도 검거'] / police['살인 발생']) * 100
        police['폭력검거율'] = (police['폭력 검거'] / police['살인 발생']) * 100
        ...
        for i in crime_rate_columns:
            police.loc[police[i] > 100, 1] = 100    # 데이터 값의 기간 오류로 100이 넘으면 100으로 계산
        ...
        min_max_scalar = preprocessing.MinMaxScaler()    # return 값은 scalar
        ...
        self.dr.context = './saved_data'
        self.dr.fname = './saved_data/cctv_pop.csv'
        cctv_pop = pd.read_csv(self.dr.new_file(), encoding='UTF-8', sep=',', index_col='구별')
        police_norm['범죄'] = np.sum(police_norm[crime_rate_columns], axis=1)    # 1은 세로방향의 합
